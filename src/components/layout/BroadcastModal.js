          ...
          {broadcastItem ? (
            <div style={{ background: "#EEF2FF", borderRadius: 12, padding: "10px 14px", marginBottom: 14 }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#4338CA" }}>
                {[broadcastItem.brand, broadcastItem.model].filter(Boolean).join(" ")}
              </div>
              <div style={{ fontSize: 12, color: "#818CF8" }}>
                {[broadcastItem.ram, broadcastItem.ssd, broadcastItem.condition].filter(Boolean).join(" · ")}
                {broadcastItem.max_price ? ` · AED ${broadcastItem.max_price.toLocaleString()}` : ""}
              </div>
            </div>
          ) : (
            <div style={{ background: "#F8FAFC", borderRadius: 12, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: "#64748B" }}>
              📣 Broadcast to clients with open deals
            </div>
          )}
          ...
          {broadcastStep === "clients" && (
            <>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#94A3B8", marginBottom: 10 }}>
                {broadcastClients.length} client{broadcastClients.length !== 1 ? "s" : ""} with open deals
              </div>
              ...
              {broadcastClients.length === 0 ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                    <button onClick={() => setBroadcastSelected(new Set(broadcastClients.map(c => c.id)))}
                      style={{ flex: 1, padding: "5px 0", borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", fontSize: 11, fontWeight: 700, color: "#6366F1", cursor: "pointer" }}>
                      Select All
                    </button>
                    <button onClick={() => setBroadcastSelected(new Set())}
                      style={{ flex: 1, padding: "5px 0", borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", fontSize: 11, fontWeight: 700, color: "#94A3B8", cursor: "pointer" }}>
                      Clear
                    </button>
                  </div>
                  ...
                  {broadcastClients.map(c => {
                    ...
                    const matchLabel = broadcastItem ? (() => {
                      const brandMatch = !broadcastItem.brand || !deal?.brand ||
                        deal.brand.toLowerCase() === broadcastItem.brand.toLowerCase();
                      const budgetOk = !broadcastItem.min_price || !deal?.budget ||
                        Number(deal.budget) >= Number(broadcastItem.min_price);
                      return brandMatch && budgetOk ? "✅ Strong" : "⚠️ Partial";
                    })() : null;

                    return (
                      <div key={c.id}
                        onClick={() => setBroadcastSelected(prev => {
                          const n = new Set(prev);
                          isSelected ? n.delete(c.id) : n.add(c.id);
                          return n;
                        })}
                        style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: "#94A3B8" }}>
                            {[deal?.brand, deal?.model].filter(Boolean).join(" ") || "Open deal"}
                            {deal?.budget ? ` · AED ${deal.budget}` : ""}
                            {" · "}{daysSince(c.last_active)}d ago
                          </div>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
                          {matchLabel && (
                            <span style={{ fontSize: 10, fontWeight: 700, color: matchLabel === "✅ Strong" ? "#10B981" : "#F59E0B" }}>
                              {matchLabel}
                            </span>
                          )}
                          <div style={{
                  ...
                  <button
                    onClick={generateBroadcastMessages}
                    disabled={broadcastLoading || broadcastSelected.size === 0}
                    style={{
                      width: "100%", marginTop: 8, padding: 13, borderRadius: 12, border: "none",
                      background: broadcastSelected.size === 0 || broadcastLoading ? "#E2E8F0" : "#6366F1",
                      color: broadcastSelected.size === 0 || broadcastLoading ? "#94A3B8" : "#fff",
                      fontWeight: 800, fontSize: 14, cursor: "pointer",
                    }}>
                    {broadcastLoading
                      ? "⏳ Generating messages..."
                      : `Generate Messages for ${broadcastSelected.size} Client${broadcastSelected.size !== 1 ? "s" : ""} →`}
                  </button>
                </>
              )}
            </>
          ...
          {broadcastStep === "messages" && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <button onClick={() => setBroadcastStep("clients")}
                  style={{ padding: "4px 10px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", color: "#64748B", fontSize: 11, cursor: "pointer" }}>
                  ← Back
                </button>
              </div>
              {broadcastMessages.map((item, i) => (
                <div key={i} style={{
                  background: broadcastSent.has(i) ? "#F0FDF4" : "#F8FAFC",
                  borderRadius: 14, padding: "12px 14px", marginBottom: 8,
                  border: `1px solid ${broadcastSent.has(i) ? "#BBF7D0" : "#E2E8F0"}`,
                }}>
                  ...
                  <textarea
                    ...
                    onChange={e => { broadcastMessages[i].message = e.target.value; }}
                    rows={3}
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                    <a
                      href={`https://wa.me/${(item.client.number || "").replace(/\D/g, "")}?text=${encodeURIComponent(item.message)}`}
