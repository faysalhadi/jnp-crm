...
function AskClaudeTab() {
  ...
  const [mode, setMode] = useState("ask"); // "ask" | "stocklist"
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
      ...
      {/* ── Mode toggle ── */}
      <div style={{ padding: "10px 12px 0", background: "#fff", borderBottom: "1px solid #F1F5F9" }}>
        <div style={{ display: "flex", borderRadius: 10, overflow: "hidden", border: "1px solid #F1F5F9" }}>
          <button onClick={() => setMode("ask")}
            style={{ flex: 1, padding: "9px 0", fontSize: 12, fontWeight: 700, border: "none", cursor: "pointer",
              background: mode === "ask" ? "#534AB7" : "#F8FAFC",
              color: mode === "ask" ? "#fff" : "#64748B" }}>
            🤖 Ask Claude
          </button>
          <button onClick={() => setMode("stocklist")}
            style={{ flex: 1, padding: "9px 0", fontSize: 12, fontWeight: 700, border: "none", cursor: "pointer",
              background: mode === "stocklist" ? "#534AB7" : "#F8FAFC",
              color: mode === "stocklist" ? "#fff" : "#64748B" }}>
            📋 Stock List
          </button>
        </div>
      </div>
