...
export default function PartsCompatibilityTab() {
  ...
  function unverifiedCount(model) {
    ...
    return parts.filter(p => !p.verified).length;
  }
  ...
  async function handleVerify(part) {
    ...
    setPartsMap(prev => {
      const updated = { ...prev };
      updated[part.model_id] = (updated[part.model_id] || []).map(p =>
  ...
  function openEdit(part) {
    setEditPart(part);
    setEditForm({ spec: part.spec, connector: part.connector || "", wattage: part.wattage || "", notes: part.notes || "" });
  }
  ...
  async function saveEdit() {
    ...
    setSaving(true);
    await supabase.from("parts_compatibility").update({
      spec: editForm.spec, connector: editForm.connector,
      wattage: editForm.wattage, notes: editForm.notes,
    }).eq("id", editPart.id);
    setPartsMap(prev => {
      const updated = { ...prev };
      updated[editPart.model_id] = (updated[editPart.model_id] || []).map(p =>
  ...
  async function openAddPart(partType) {
    ...
    setEditPart({ id: null, model_id: selectedModel.id, part_type: partType.id, spec: "", connector: "", wattage: "", notes: "", _isNew: true, _label: partType.label });
    setEditForm({ spec: "", connector: "", wattage: "", notes: "" });
  }
  ...
  async function saveNewPart() {
    ...
    const { data } = await supabase.from("parts_compatibility").insert({
      ...
      spec: editForm.spec || "TBC", connector: editForm.connector,
      wattage: editForm.wattage, notes: editForm.notes,
    }).select().single();
    if (data) {
      setPartsMap(prev => {
        const updated = { ...prev };
        if (!updated[editPart.model_id]) updated[editPart.model_id] = [];
        updated[editPart.model_id] = [...updated[editPart.model_id], data];
        return updated;
      });
    }
    setSaving(false);
                ...
                {visibleParts.map(part => {
                  ...
                  return (
                    <div key={part.id} style={{ background: "#fff", borderRadius: 14, padding: "14px 14px 12px", border: "1.5px solid #F1F5F9", position: "relative", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                      ...
                      <div style={{ fontSize: 13, fontWeight: 800, color: "#0F172A", marginBottom: 6, lineHeight: 1.4 }}>{part.spec}</div>
      ...
      {editPart && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 500, display: "flex", alignItems: "flex-end" }}>
          <div style={{ background: "#fff", width: "100%", maxWidth: 500, margin: "0 auto", borderRadius: "20px 20px 0 0", padding: "20px 20px 40px" }}>
            ...
            {[
              { key: "spec", label: "Spec", placeholder: "e.g. 14\" FHD 1920x1080 IPS 30-pin eDP" },
